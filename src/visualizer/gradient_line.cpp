...
GradientLine::GradientLine() : function_(Line), function_domain_(vec2(0.0, 1.0)), function_range_(0.0, 1.0) {
  ...
  color_positions_.push_back(0.5);
}

GradientLine::GradientLine(const vector<Color> &colors, line_function function, vec2 domain) : function_(function), function_domain_(domain), function_range_(ApproximateFunctionRange(function, domain)) {
  colors_ = colors;

  double num_colors = colors.size();
  double div_dist = 1.0 / num_colors;
  for (size_t div = 0; div < num_colors; div ++){
    color_positions_.push_back(div * div_dist);
  }
}

GradientLine::GradientLine(const vector<Color> &colors, const vector<double> &positions, line_function function, vec2 domain) : function_(function), function_domain_(domain), function_range_(ApproximateFunctionRange(function, domain)) {
  colors_ = colors;
...
void GradientLine::Step(double proportion) {
  for (size_t i = 0; i < color_positions_.size(); i++){
    ...
    if (color_positions_[i] > 1){
      color_positions_[i] -= 1;
    }
  }
}
